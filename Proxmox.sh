...
Export_vm() {
    ...
    echo "# ===========================================" > "$METADATA_FILE"
    echo "# Proxmox VM Metadata" >> "$METADATA_FILE"
    echo "# Generated by Proxmox.sh" >> "$METADATA_FILE"
    echo "# ===========================================" >> "$METADATA_FILE"
    echo >> "$METADATA_FILE"

    echo "# VM Information" >> "$METADATA_FILE"
    echo "VMID=$VMID1" >> "$METADATA_FILE"
    echo "NAME=$VM_name" >> "$METADATA_FILE"
    echo "BIOS=$(qm config "$VMID1" | awk -F': ' '/^bios:/ {print $2}')" >> "$METADATA_FILE"
    echo "MACHINE=$(qm config "$VMID1" | awk -F': ' '/^machine:/ {print $2}')" >> "$METADATA_FILE"
    echo "OSTYPE=$(qm config "$VMID1" | awk -F': ' '/^ostype:/ {print $2}')" >> "$METADATA_FILE"
    echo "MEMORY=$(qm config "$VMID1" | awk -F': ' '/^memory:/ {print $2}')" >> "$METADATA_FILE"
    echo "CORES=$(qm config "$VMID1" | awk -F': ' '/^cores:/ {print $2}')" >> "$METADATA_FILE"
    echo "SOCKETS=$(qm config "$VMID1" | awk -F': ' '/^sockets:/ {print $2}')" >> "$METADATA_FILE"

    echo >> "$METADATA_FILE"
    echo "# Disk Information" >> "$METADATA_FILE"
    ...
    if [[ "$disk" == "all" ]]; then
        ...
        for disk_name in $disk_names; do
            ...
            DiskConfig=$(qm config "$VMID1" | awk -F': ' -v disk="$disk_name" '$1==disk {print $2}')

            DiskRef=$(echo "$DiskConfig" | cut -d',' -f1)

            DiskOptions=$(echo "$DiskConfig" | cut -d',' -f2-)

            DiskStorage=$(echo "$DiskRef" | cut -d':' -f1)

            DiskImage=$(echo "$DiskRef" | cut -d':' -f2)

            echo "${disk_name}|${DiskStorage}|${DiskImage}|${DiskOptions}" >> "$METADATA_FILE"
    ...
    echo
    echo "📄 Saving original Proxmox configuration..."
    qm config "$VMID1" > "${VM_name}.qm.conf"
